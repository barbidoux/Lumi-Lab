...
from utils.robust_imports import (
    smart_sentence_segmentation,
    get_validation_modules,
    is_nltk_available,
    retry_on_failure
)
from utils.tokenizer_metrics import (
    TokenizerTrainingMetrics,
    setup_enhanced_logging
)
from utils.adaptive_estimation import AdaptiveTokenEstimator
                ...
                for line_num, line in enumerate(f, 1):
                    try:
                        ...
                        for sentence in sentences:
                            ...
                            if 5 <= len(sentence) <= 2048:  # Same filtering as streaming
            ...
            with gzip.open(shard_path, 'rt', encoding='utf-8') as f:
                for line in f:
                    ...
                    try:
                        ...
                        for sentence in sentences:
                            sentence = sentence.strip()
                            if 5 <= len(sentence) <= 2048:  # Same filtering as analysis and training
                                valid_sentences.append(sentence)
...
def train_sentencepiece_tokenizer(
    ...
    if special_tokens:
        ...
        spm_args.append(f'--control_symbols={control_symbols}')
...
def main():
    ...
    parser.add_argument("--model-type", default="unigram", choices=["unigram", "bpe", "word", "char"],
                       help="SentencePiece model type")
    parser.add_argument("--character-coverage", type=float, default=0.9995, help="Character coverage")
    parser.add_argument("--normalization-rule", default="nfkc", help="Text normalization rule")
    parser.add_argument("--max-training-sentences", type=int, default=10000000,
                       help="Maximum sentences for training")
    parser.add_argument("--max-sentence-length", type=int, default=32768,
                       help="Maximum sentence length (longer sentences are truncated)")
    parser.add_argument("--calibration-samples", type=int, default=1000,
                       help="Number of documents for estimation calibration")
    ...
    estimator = AdaptiveTokenEstimator(initial_estimate=4.0)

    # Calibrate estimation
    old_estimate = estimator.current_estimate
    estimator.calibrate_from_corpus(manifest, corpus_dir, args.calibration_samples)

    if abs(estimator.current_estimate - old_estimate) > 0.1:
        metrics.log_estimation_calibration(old_estimate, estimator.current_estimate, args.calibration_samples)
    ...
    corpus_stats = analyze_corpus_statistics(manifest, corpus_dir, metrics, estimator)
    ...
    sentence_iterator = stream_sentences_from_shards(
        manifest, corpus_dir, metrics, estimator, args.max_training_sentences
    )
    ...
    training_stats = train_sentencepiece_tokenizer(
        ...
        max_training_sentences=args.max_training_sentences,
    ...
    if validation_modules:
        ...
        try:
            ...
            dataset_validation = token_counter.validate_dataset_quality(
                manifest, corpus_dir, sample_rate=0.1
            )
